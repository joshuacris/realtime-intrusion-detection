...
def tune_model(base_model, X_train, y_train, n_iter=50, cv=3):
    param_dist = {
        'hidden_layer_sizes': [(32,), (64,), (128,), (256,), (64, 32), (128, 64), (128, 32),
                               (256, 128), (256, 32), (128, 64, 32), (256, 128, 64), (256, 128, 64, 32)],
        'activation': ['relu', 'tanh'],
        'learning_rate_init': [1e-2, 1e-3, 5e-4, 3e-4, 1e-4],
...
def per_attack_breakdown(raw_test_path, y_test, y_pred, y_prob):
    ...
    ax.set_title('MLP — Per-Attack-Category Metrics')
    ax.set_ylabel('Score')
    ax.set_ylim(0, 1.05)
...
def plot_feature_importance(model, X_test, y_test, X_train, top_n=20):
    ...
    ax.set_title('MLP — Top 20 Feature Importances (Permutation)')
